...
pub const BuiltinAttrDoc = struct {
...
pub const builtin_attrs = std.StaticStringMap(BuiltinAttrDoc).initComptime(.{
    .{
        "@rendering",
        BuiltinAttrDoc{
            .name = "@rendering",
...
fn hoverForNode(arena: std.mem.Allocator, parse: *Parse, node: anytype, source: []const u8) !?Hover {
    ...
    switch (kind) {
        ...
        .identifier => {
            // Shorthand forms use a plain identifier: `{class}` / `@{allocator}`.
            return try hoverForShorthandIdentifier(arena, parse, node, source);
        },
        else => return null,
    }
}

fn hoverForShorthandIdentifier(
    ...
    const name = nodeText(node, source);
    switch (NodeKind.fromNode(parent)) {
        .zx_shorthand_attribute => {
            const tag = enclosingTagNameFromAttr(parent, source) orelse "";
            const md = try attributeMarkdown(arena, tag, name) orelse return null;
            return .{ .markdown = md, .start_byte = node.startByte(), .end_byte = node.endByte() };
        },
        .zx_builtin_shorthand_attribute => {
            ...
            const md = try builtinAttributeMarkdown(arena, buf[0 .. name.len + 1]) orelse return null;
            return .{ .markdown = md, .start_byte = node.startByte(), .end_byte = node.endByte() };
        },
        else => return null,
    }
...
fn elementMarkdown(arena: std.mem.Allocator, name: []const u8) !?[]const u8 {
    // ZX components / custom elements are not HTML — don't show MDN docs for
    // case-insensitive collisions like `<Button>` → `<button>`.
    if (elements.isCustomOrComponent(name)) return null;
...
fn attributeMarkdown(arena: std.mem.Allocator, tag: []const u8, attr: []const u8) !?[]const u8 {
    if (elements.isCustomOrComponent(tag)) return null;
...
pub fn builtinAttributeMarkdown(arena: std.mem.Allocator, name: []const u8) !?[]const u8 {
...
fn enclosingTagNameFromAttr(attr_node: anytype, source: []const u8) ?[]const u8 {
    var current = attr_node.parent();
    while (current) |n| : (current = n.parent()) {
        switch (NodeKind.fromNode(n)) {
            .zx_start_tag, .zx_self_closing_element => {
                const name_node = n.childByFieldName("name") orelse return null;
